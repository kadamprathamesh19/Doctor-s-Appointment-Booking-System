        ...
        {appointments.map((item, index) => (
          <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={index}>
            ...
            <div className='flex-1 text-sm text-zinc-600 '>
              ...
              <p className='text-xs mt-1'><span className='text-sm text-neutral-700 font-medium' >Date & Time:</span> {slotDateFormat(item.slotDate)} | {item.docData.slotTime}</p>
            </div>
            <div></div>
            <div className='flex flex-col gap-2 justify-end'>
              ...
              {!item.cancelled && !item.isCompleted && <button onClick={() => cancelAppointment(item._id)} className='text-sm text-stone-500 text-center sm:min-w-48 py-2 border rounded hover:bg-red-600 hover:text-white transition-all duration-300'>Cancle Appoitment</button>}
              {!item.cancelled && !item.isCompleted && <button className='sm:min-w-48 py-4 border border-red-500 rounded text-red-500'>Appointment Canceled</button>}
